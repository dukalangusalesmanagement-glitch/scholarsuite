...
import { Plus, Edit3, Trash2, Loader2, Mail, Phone } from "lucide-react";
...
export default function Teachers() {
  const { t } = useLang();
  const { rows, loading, create, update, remove } = useResource("teachers", { orderBy: "created_at" });
  ...
  const filtered = rows.filter((r) =>
    [r.full_name, r.email, r.specialization].join(" ").toLowerCase().includes(search.toLowerCase())
  );
  ...
  return (
    <div>
      <PageHeader
        title={t.teachers}
        subtitle={`${rows.length} ${t.teachers.toLowerCase()}`}
        actions={
          <button
            onClick={() => open()}
      ...
      {loading ? (
        <div className="py-16 text-center"><Loader2 className="mx-auto h-5 w-5 animate-spin text-stone-400" /></div>
      ) : filtered.length === 0 ? (
        <div className="py-16 text-center text-stone-400">{t.noData}</div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((r) => (
            <div key={r.id} className="bg-white rounded-2xl border border-stone-200 p-5 hover:border-emerald-700 transition">
              <div className="flex items-start gap-3 mb-4">
                <div className="h-12 w-12 rounded-full flex items-center justify-center text-white font-semibold" style={{ background: "var(--green-700)" }}>
                  {(r.full_name || "?").slice(0, 2).toUpperCase()}
                </div>
                <div className="flex-1 min-w-0">
                  <h3 className="font-medium text-stone-900 truncate">{r.full_name}</h3>
                  <p className="text-xs text-stone-500">{r.specialization || "—"}</p>
                  <p className="text-[10px] uppercase tracking-wider text-stone-400 mt-1">{r.employee_no}</p>
                </div>
                <div className="flex gap-1">
                  <button onClick={() => open(r)} className="p-1.5 rounded hover:bg-stone-100 text-stone-500"><Edit3 className="h-4 w-4" /></button>
                  <button onClick={() => confirm(t.deleteConfirm) && remove(r.id)} className="p-1.5 rounded hover:bg-red-50 text-stone-500 hover:text-red-600"><Trash2 className="h-4 w-4" /></button>
                </div>
              </div>
              <div className="space-y-1.5 text-xs text-stone-600">
                {r.email && <p className="flex items-center gap-2"><Mail className="h-3.5 w-3.5" /> {r.email}</p>}
              ...
              {r.salary > 0 && (
                <div className="mt-4 pt-3 border-t border-stone-100 flex items-baseline justify-between">
                  <span className="text-xs text-stone-500">{t.salary}</span>
                  <span className="display text-lg" style={{ color: "var(--green-950)" }}>{formatTZS(r.salary)}</span>
                </div>
              )}
            </div>
          ))}
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? t.edit : `${t.addNew} ${t.teachers}`} size="lg">
        <form onSubmit={save} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <Field label="Employee No.">
              <input className={inputClass} value={form.employee_no} onChange={(e) => setForm({ ...form, employee_no: e.target.value })} />
            </Field>
